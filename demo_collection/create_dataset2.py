...
def main():
    ...
    for (run_idx, path) in enumerate(Path(folder).glob("run*")):
        ...
        # print('load: ', len(camera_data[0]), len(camera_data[1]), camera_data[0].shape)
        ...
        # print('checking ', len(actions), len(proprio_ee), len(image_color_data[0]), len(image_color_data[1]) )
        ...
        for camera_id in camera_ids:
            ep_data_grp = ep_grp.create_dataset(
                f"camera_{camera_id}_color", data=image_color_data[camera_id]
            )
